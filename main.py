...
def auto_update_daemon(current_file_path):
    ...
    # Check if current_file_path is a regular file (not a pipe or special file)
    if not os.path.isfile(current_file_path):
        logger.warning(f"Skipping auto-update: '{current_file_path}' is not a regular file (pipe/special file detected)")
        logger.warning("To enable auto-updates, download the script to a file first:")
        logger.warning("  curl -sSL https://github.com/maxmusdotnet/XProxyCon/blob/main/main.py?raw=1 -o installer.py")
        logger.warning("  python3 installer.py")
        return
    ...
    while True:
        try:
            ...
            req = urllib.request.Request(
                UPDATE_URL,
                headers={"User-Agent": "XProxyCon-AutoUpdater/1.2.1"}
            )
    ...
    def _get_public_ip(self):
        ...
        for service in ip_services:
            try:
                ...
                req.add_header('User-Agent', 'XProxyCon-Installer/1.2.1')
